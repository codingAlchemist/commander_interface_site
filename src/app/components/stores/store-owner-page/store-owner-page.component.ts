...
export class StoreOwnerPageComponent implements OnInit {
  ...
  onUpdateDetails() {
    ...
    if (
      !this.isEmpty(this.ownerDetailsForm.value.username!) &&
      this.ownerDetailsForm.value.username != this.admin.username
    ) {
      username = this.ownerDetailsForm.value.username!;
      console.log(`username input ${username}`);
    }
    if (
      !this.isEmpty(this.ownerDetailsForm.value.email!) &&
      this.ownerDetailsForm.value.email != this.admin.email
    ) {
      email = this.ownerDetailsForm.value.email!;
    }
    if (
      !this.isEmpty(this.ownerDetailsForm.value.firstname!) &&
      this.ownerDetailsForm.value.firstname != this.admin.firstname
    ) {
      firstname = this.ownerDetailsForm.value.firstname!;
    }
    if (
      !this.isEmpty(this.ownerDetailsForm.value.lastname!) &&
      this.ownerDetailsForm.value.lastname != this.admin.lastname
    ) {
      lastname = this.ownerDetailsForm.value.lastname!;
    }
    if (
      !this.isEmpty(this.ownerDetailsForm.value.password!) &&
      this.ownerDetailsForm.value.password != this.admin.pass
    ) {
